...
foreach($res as $val)
{
	$el = array();
	$el['L.p.'] = $i++;
	$el['Nazwa usługi'] = $val['name'];
	$el['Liczba jedn.'] = '1.00';
	$el['Wartość netto'] = $val['value'];
	$el['VAT'] = number_format(round($val['value'] * ($tax / 100), 2), 2, '.', '');
	$el['Wartość brutto'] = number_format($val['value'] + $el['VAT'], 2, '.', '');
	$data[] = $el;
	$suma += $val['value']; $poz++;
}
$el = array();
$el['Nazwa usługi'] = 'Razem';
$el['Wartość netto'] = number_format($suma, 2, '.', '');
$el['VAT'] = number_format(round($suma * ($tax / 100), 2), 2, '.', '');
$el['Wartość brutto'] = number_format($suma + $el['VAT'], 2, '.', '');
...
if(is_array($data)) foreach($data as $val)
{
	...
	$el['Opłata'] = $val['cost'];
	$el['Opłata brutto'] = number_format(round($val['cost'] * ($tax/100) + $val['cost'], 2), 2, '.', '');
	$out[] = $el;
}
return $out;
}
